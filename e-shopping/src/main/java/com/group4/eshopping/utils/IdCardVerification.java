...
import com.group4.eshopping.exception.OnlyForException;
...
//     */
    public static Boolean IDCardValidate(String IDStr) {
        try {
            ...
            if (IDStr.length() != 15 && IDStr.length() != 18) {
                throw new OnlyForException("length");
            }
            ...
            if (!isNumeric(Ai)) {
                throw new OnlyForException("only number");
            }

            // 判断出生年月是否有效
            if (!isValidDate(Ai))
                throw new OnlyForException("date");

            // 判断地区码是否有效
            Hashtable<String, String> areacode = GetAreaCode();
            // 如果身份证前两位的地区码不在Hashtable，则地区码有误
            if (areacode.get(Ai.substring(0, 2)) == null) {
                throw new OnlyForException("area");
            }
    ...
    private static boolean isValidDate(String Ai) {
        try {
            String strYear = Ai.substring(6, 10);// 年份
            String strMonth = Ai.substring(10, 12);// 月份
            String strDay = Ai.substring(12, 14);// 日期
            if (!isDate(strYear + "-" + strMonth + "-" + strDay)) {
                throw new OnlyForException("birthday");
            }
            ...
            try {
                if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
                    ...
                    throw new OnlyForException("year");
                }
            } catch (NumberFormatException | ParseException e) {
                throw new OnlyForException("year");
            }
            if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {
                throw new OnlyForException("month");
            }
            if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
                throw new OnlyForException("date");
            }
            return true;
        } catch (OnlyForException e) {
            return false;
        }
    }
