...
const created = () => {
  service.get('/goods/change/approve',{})
      .then(function (res) {
...
function Approve(row_id){
  let goodsChangeId = dataObj.tableData_0[row_id].id
  service.post('/goods/change/approve', {
    goodsChangeId: goodsChangeId,
    isApproved: true
  })
      .then(function (res) {
        if (res.status === 200) {
...
function Jump(row_id){
  let url = '/goods_details?goodsId=' + dataObj.tableData_0[row_id].goodsId
  router.push(url)
}
created();
