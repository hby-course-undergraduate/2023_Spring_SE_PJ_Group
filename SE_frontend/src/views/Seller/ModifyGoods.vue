...
const Apply = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      formData.append('goodsId', getGoodsId())

      service.put('/goods/change', formData ,{
  ...
  service.get(url,{})
      .then(function (res) {
        if (res.status === 200) {
          ...
          ApplyForm.price = res.data.price
          ApplyForm.description = res.data.description
          ApplyForm.number = res.data.number
          ApplyForm.goodsName = res.data.goodsName
          ApplyForm.tag = res.data.tag
